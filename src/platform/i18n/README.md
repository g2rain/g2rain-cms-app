...
登录后拉取 `CMS` 标签文案包；HTTP 自动带 `Accept-Language`（当前 `localeStore.locale`，如 `zh-CN`）。
...
- 文案编码与后台 `i18n_message.message_code` 一致，`tag` 固定为 `CMS`（见 `runtime/api/i18n.api.ts`）。
- 拉包：`GET /api/infra/i18n_message/locale?tag=CMS&locale=zh-CN`。
